class HgoFileManager {
    ...
    #initExplorer() {
        ...
        if (Cookies.get('hgo-fm-current-view-' + this.#continerId))
            this.#jsSelectContinerDiv.classList.add(Cookies.get('hgo-fm-current-view-' + this.#continerId));
        ...
        this.#jsSelect = new SelectionArea({
            selectables: ['#' + this.#continerId + ' .hgo-fm-fsitems > .fsitem'],
            boundaries: [this.#jsSelectContinerDiv]
        }).on('beforestart', ({ store, event }) => {
            let contextMenu = $('#' + this.#continerId + ' .hgo-fm-fsitems .context');
    ...
    #fillFolderContent = function (data) {
        if (data.error) {
        ...
        data.files.forEach((file) => {
            ...
            fsItemImg.classList.add('lozad');
            fsItemImg.src = '/hgofilemanager/images/loading.gif';
            fsItemImg.setAttribute('data-src', this.#apiUrl + `?id=${this.#continerId}&command=filePreview&parameters=${currentPath + file}`);
            fsItemDiv.appendChild(fsItemImg);
    ...
    #checkIfAnyItemSelected = function (itemtype) {
    ...
    #fileManager() {
        return document.getElementById(this.#continerId);
    }
    ...
    getCurrentPath = function () {
        let path = Cookies.get('hgo-fm-current-path-' + this.#continerId);
        if (path)
            return path.replace(/\\+$/, '') + "\\";
        return "";
    }
    ...
    setCurrentPath = function (path) {
        if (path)
            Cookies.set('hgo-fm-current-path-' + this.#continerId, path, { expires: 365 });
    }
    ...
    getFolderContent = function (folder, addToHistory = true) {
        ...
        this.#WaitingPanel();
        $.post(this.#apiUrl, { id: this.#continerId, command: "getFolderContent", parameters: folder }, function (data) {
            self.#WaitingPanel();
    ...
    search = function (query) {
        let self = this;
        this.#WaitingPanel();
        $.post(this.#apiUrl, { id: this.#continerId, command: "Search", parameters: JSON.stringify({ Path: this.getCurrentPath(), Query: query }) }, function (data) {
            self.#WaitingPanel();
            self.#fillFolderContent(data);
        }, 'json');
    }
    ...
    createNewFolder = function () {
        ...
        let self = this;
        this.#WaitingPanel();
        $.post(this.#apiUrl, { id: this.#continerId, command: "CreateNewFolder", parameters: JSON.stringify({ Path: this.getCurrentPath(), FolderName: newFolderName }) }, function (data) {
            self.#WaitingPanel();
            self.#fillFolderContent(data);
        }, 'json');
    }
    ...
    createNewFile = function () {
        ...
        let self = this;
        this.#WaitingPanel();
        $.post(this.#apiUrl, { id: this.#continerId, command: "CreateNewFile", parameters: JSON.stringify({ Path: this.getCurrentPath(), FileName: newFileName }) }, function (data) {
            self.#WaitingPanel();
            self.#fillFolderContent(data);
        }, 'json');
    }
    ...
    deleteSelectedItems = function () {
        ...
        if (confirm("Are you sure you want to delete selected items?")) {
            let self = this;
            this.#WaitingPanel();
            $.post(this.#apiUrl, { id: this.#continerId, command: "DeleteItems", parameters: JSON.stringify({ Path: this.getCurrentPath(), Items: selectedItems }) }, function (data) {
                self.#WaitingPanel();
                self.#fillFolderContent(data);
            }, 'json');
        }
    ...
    renameSelectedItems = function () {
        ...
        let self = this;
        this.#WaitingPanel();
        $.post(this.#apiUrl, { id: this.#continerId, command: "RenameItems", parameters: JSON.stringify({ Path: currentPath, Items: selectedItems, NewName: newName }) }, function (data) {
            self.#WaitingPanel();
            self.#fillFolderContent(data);
        }, 'json');
    }
    ...
    zipSelectedItems = function () {
        ...
        let self = this;
        this.#WaitingPanel();
        $.post(this.#apiUrl, { id: this.#continerId, command: "ZipItems", parameters: JSON.stringify({ Path: this.getCurrentPath(), Items: selectedItems, FileName: zipFileName }) }, function (data) {
            self.#WaitingPanel();
            self.#fillFolderContent(data);
        }, 'json');
    }
    ...
    extractSelectedItems = function () {
        ...
        let self = this;
        this.#WaitingPanel();
        $.post(this.#apiUrl, { id: this.#continerId, command: "ExtractItems", parameters: JSON.stringify({ Path: this.getCurrentPath(), Items: selectedItems }) }, function (data) {
            self.#WaitingPanel();
            self.#fillFolderContent(data);
        }, 'json');
    }
    ...
    showFileContent = function () {
        ...
        for (let i = 0; i < selectedItems.length; i++) {
            window.open(`${this.#apiUrl}?id=${this.#continerId}&command=ShowFileContent&parameters=${selectedItems[i]}`, '_blank');
        }
    }
    ...
    downloadFile = function () {
        ...
        for (let i = 0; i < selectedItems.length; i++) {
            window.open(`${this.#apiUrl}?id=${this.#continerId}&command=DownloadItem&parameters=${selectedItems[i]}`, '_blank');
        }
    }
    ...
    pasteItems = function () {
        if (this.#clipboard.length > 1) {
            ...
            $.post(this.#apiUrl, { id: this.#continerId, command: action, parameters: JSON.stringify({ Path: this.getCurrentPath(), Items: this.#clipboard }) }, function (data) {
                self.#WaitingPanel();
                self.#fillFolderContent(data);
            }, 'json');
        }
    ...
    toggleFilesListView = function () {
        if (this.#jsSelectContinerDiv.classList.contains('list')) {
            ...
            Cookies.set('hgo-fm-current-view-' + this.#continerId, '', { expires: 365 });
        }
        else {
            this.#jsSelectContinerDiv.classList.add('list');
            Cookies.set('hgo-fm-current-view-' + this.#continerId, 'list', { expires: 365 });
        }
    }
    ...
    showUploadPanel = function () {
        ...
        let dropZone = new Dropzone(uploadPanel, {
            url: this.#apiUrl + `?id=${this.#continerId}&command=upload&parameters=${this.getCurrentPath()}`,
            parallelUploads: 1,
            chunking: true,
